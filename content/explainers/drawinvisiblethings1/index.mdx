...
This is partly because programming is an inherently abstract activity.  
It has to be.  Taken literally, programming involves running enormously complex sequences of electrical currents on a microscopic scale inside our machines.

Microchip world isn't human friendly. We can't see what's going on or control what happens there without a thick layer of symbolic software in the middle.  
All of our programming languages *are* the metaphor. `const fruit = banana`  _maps onto_ a specfic set of electrical pulses firing deep inside the hardware. <Tooltip tiptext={<span>Here's a terribly nerdy MIT PhD thesis about it if you want to go down the rabbit hole... <Link to="http://alumni.media.mit.edu/~mt/thesis/mt-thesis.html">Programming with Agents: New metaphors for thinking about computation</Link></span>}>6</Tooltip>
...
The point here is that metaphor is native to programming.
These illustrations are just one more layer on the human-friendly stack of abstractions. 
