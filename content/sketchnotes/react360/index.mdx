---
date: 2019-05-14
title: Building VR Apps with React360 Notes
thumbnail: ./thumb@2x.png
description: null
slug: react360
category: sketchnotes
published: true
---

import Link from '../../../src/components/link.js'

<div style ={{ maxWidth: '660px', margin: '0 auto' }}>

Turns out you can build almost anything in React now... including VR and AR apps inside a web browser.

There's a version of React called '[React 360](https://facebook.github.io/react-360/)' that simplifies the otherwise niche and overly-complex challenge of making things fly around in virtual 3D space.

[Tomasz Lakomy](https://egghead.io/instructors/tomasz-lakomy) is putting together an egghead course all about it.

<Link to="https://egghead.io/courses/vr-applications-using-react-360">

![](./vr-applications-using-react-360.jpg)

</Link>

So I've been sketching out notes on some of the basic concepts while watching his lessons.

![](./react360_p1@2x.png)

![](./react360_p2@2x.png)

![](./react360_p3@2x.png)

![](./react360_p4@2x.png)

---

<h2 style="text-align: center;">The Full Sketchnote</h2>

<br />

<div style="border: 1px solid #e8f3f7; box-sizing: border-box; border-radius: 4px;
  box-shadow: 0px 8px 16px -12px rgba(120, 134, 138, 0.75);">

![](./react360_full@2x.png)

</div>

Want to learn more?
A high-res version of the sketchnote plus the full [VR Applications Using React360](https://egghead.io/courses/vr-applications-using-react-360) course will be out soon. Best to keep an eye on that page if it's a topic you're into.
</div>
